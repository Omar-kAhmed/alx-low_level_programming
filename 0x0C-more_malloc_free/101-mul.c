...
/**
 * numstrchk - checks arg array to see if the are numeric strings, 
converts
 * from ascii to byte int, and returns their length. Segfault on null 
pointer.
 *
 * @s: string to check
 *
...
/**
 * _calloc_buffer - allocate a block of memory of size * num and init to 
'0'
 *
...
int main(int ac, char **av)
{
	...
	for (i = lenres - 1, len1--; len1 >= 0; len1--, i += len2 - 1)
		for (j = len2 - 1; j >= 0; j--, i--)
		{
			...
			res[i - 1] = (av[1][len1] * av[2][j] / 10) + res[i 
- 1];
			if (res[i] > '9')
			{
				res[i] -= 10;
